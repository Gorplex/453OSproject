...
void checkArgs(int argc, char **argv){
    if(argc < 2){
        fprintf(stderr, "Usage: %s [OPTION] FILESYS [PATH]\n", argv[0]);
        exit(EXIT_FAILURE);
    }else{
...
void read_block(uint32_t block, uint16_t offset, uint8_t* data, uint16_t size) {
    read_data(block*2+offset/512, offset%512, data, MIN(512-(offset%512), size));   
    if(size>=512){
        read_data(block*2+1, offset%512, data+512-offset, size-512-offset);
    }
}
/*
void read_block(uint32_t block, uint16_t offset, uint8_t* data, uint16_t size) {
    if(offset < 512){
        read_data(block*2, offset, data, MIN(512-(offset%512), size));   
    }
    if(size>=512 || offset >=512 ){
        read_data(block*2+1, MAX(offset-512, 0), data+MAX(512-offset, 0), size-MAX(512-offset,0));
    }
}
*/
